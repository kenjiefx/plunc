...
function processRepeats(
  ...
  const repeatElements =
    elementCtx.querySelectorAll<HTMLElement>("[plunc-repeat]");
  const parentElement = repeatElements[0];
  const template = parentElement.innerHTML;
  parentElement.innerHTML = "";
  const repeatExpression = parentElement.getAttribute("plunc-repeat");
  if (!repeatExpression) return;
  console.log("Repeat Expression:", repeatExpression);
  const [referenceObjectString, itemAlias] = repeatExpression
    .split(" as ")
    .map((part) => part.trim());
  const referenceObject = resolveExpression(dataCtx, referenceObjectString);
  const howManyRepeats = referenceObject.length;
  let finalHtml = "";
  for (let i = 0; i < howManyRepeats; i++) {
    const itemData = referenceObject[i];
    finalHtml += template;
  }
  parentElement.innerHTML = finalHtml;
  document.body.appendChild(elementCtx);
}
...
setTimeout(() => {
  ...
  if (template) {
    temporaryElement.innerHTML = template.innerHTML;
    processRepeats(temporaryElement, dataContext);
  }
}, 500);
...
type ComponentScope = { [key: string]: any };
class PluncElement {
...
type ExpressionResolver = typeof resolveExpression;
 ...
 */
function getResolveType(expression: any): ResolveType {
...
function getParentObjAsObject(base: object, expression: string) {
  const parentObjExp = getParentObjectExp(expression);
  return resolveExpression(base, parentObjExp);
}

function getChildObjectExp(expression: string) {
  let pieces = expression.split(".");
  return pieces[pieces.length - 1];
}

function areTwoExpressionsTheSame(
  scope: ComponentScope,
  expression: string,
  comparator: string,
): boolean {
