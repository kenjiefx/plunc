...
export const resolveExpression = (
  ...
  expression: string,
  element: PluncElementInterface<HTMLElement> | null = null,
) => {
    ...
    case "function":
      ...
      if (expressionTest.length > 1) {
        let refObject = resolveExpression(
          scope,
          getParentObjectExpression(structure[0]),
        );
...
function evalObject(scope: { [key: string]: any }, expression: string) {
  if (expression === "$scope") {
    return scope;
  }
...
function invokeFunction(
  ...
  expression: string,
  element: PluncElementInterface<HTMLElement> | null,
): any {
  /**
...
function getParentObjectExpression(expression: string) {
...
export function getParentObjectAsObject(base: object, expression: string) {
  const parentObjExp = getParentObjectExpression(expression);
  return resolveExpression(base, parentObjExp);
}

export function getChildObjectExpression(expression: string) {
  let pieces = expression.split(".");
